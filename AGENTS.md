...
- **Frontend:** Built and published from GitHub Actions with `wrangler pages deploy` (`master` → production, `beta` → `beta.polyratings.pages.dev`, PRs → `pr-<number>` with `VITE_API_URL` pointing at the Worker preview). Deploy builds with `NX_NO_CLOUD=true` and Vite `--mode` from the environment (`production` / `beta` / `dev`) so Nx Cloud cannot reuse CI’s default-mode dist. Disable Cloudflare Pages Git automatic deployments so only this workflow publishes. Closed same-repo PRs delete those Pages deployments and point the Worker alias at a 410 stub.
