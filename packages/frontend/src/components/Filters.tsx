...
const FilterRenderFunction:React.ForwardRefRenderFunction<FilterHandle, FilterProps> = ({ teachers, onUpdate, className }, ref) => {
    ...
    // Component State
    const [departmentFilters, setDepartmentFilters] = useState<{name:string, state:boolean}[]>(previousState?.departmentFilters ?? []);
    const [avgRatingFilter, setAvgRatingFilter] = useState<[number, number]>(previousState?.avgRatingFilter ?? [0, 4]);
    const [studentDifficultyFilter, setStudentDifficultyFilter] = useState<[number, number]>(previousState?.studentDifficultyFilter ?? [0, 4]);
    const [materialClearFilter, setMaterialClearFilter] = useState<[number, number]>(previousState?.materialClearFilter ?? [0, 4]);
    const [numberOfEvaluationsFilter, setNumberOfEvaluationsFilter] = useState<[number, number]>(previousState?.numberOfEvaluationsFilter ?? [1, 2]);
    ...
    const sortingMap:{[key in SortingOptions]:(a: TeacherEntry, b: TeacherEntry) => number} = {
        ...
        relevant: () => { throw new Error('not a sort'); },
