...
export function EvaluateTeacherForm({
 teacher, setTeacher, closeForm, overrideSubmitHandler, innerRef,
 ...
 register, handleSubmit, watch, formState: { errors },
    ...
    return (
      <form className="relative w-full" onSubmit={handleSubmit(onSubmit)} ref={innerRef}>
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between">
          {teacher
                    ...
                    <select className="h-7 rounded w-40 text-black" {...register('knownClass')}>
                      {Object.keys(teacher.reviews || {})?.map((c) => <option value={c} key={c}>{c}</option>)}
        ...
        <textarea
          {...register('reviewText', { required: { value: true, message: 'Writing a review is required' } })}
