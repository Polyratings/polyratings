...
describe("getActiveCourseIndex", () => {
    it("returns -1 when no sections are visible above threshold", () => {
        expect(getActiveCourseIndex([0, 0.1, 0.2])).toBe(-1);
        expect(getActiveCourseIndex([0, 0, 0])).toBe(-1);
        expect(getActiveCourseIndex([])).toBe(-1);
    });

    it("returns the index of the section with highest visibility above 25% threshold", () => {
        expect(getActiveCourseIndex([0.1, 0.5, 0.3])).toBe(1);
        expect(getActiveCourseIndex([0.25, 0.3, 0.7, 0.2])).toBe(2);
    ...
    it("handles equal visibility ratios by preferring the first one", () => {
        ...
        expect(getActiveCourseIndex([0.2, 0.3, 0.3])).toBe(1);
    });

    it("activates sections exactly at 25% threshold", () => {
        expect(getActiveCourseIndex([0.25, 0, 0])).toBe(0);
        expect(getActiveCourseIndex([0, 0.25, 0])).toBe(1);
    });
    ...
    it("handles the last section case (short content)", () => {
        ...
        expect(getActiveCourseIndex([0.1, 0.4, 0.3])).toBe(1);
    ...
    it("ignores sections below threshold when others are above", () => {
        expect(getActiveCourseIndex([0.1, 0.5, 0.2])).toBe(1);
        expect(getActiveCourseIndex([0.24, 0.25, 0.23])).toBe(1);
    });
});
