...
export function Home() {
    ...
    useEffect(() => {
        async function retrieveHomeData() {
            const [bestTeacher, worstTeachers] = await Promise.all([teacherService.getRandomBestTeacher(), teacherService.getRandomWorstTeachers()]);
      ...
      <div>
        ...
        <div className="justify-center pl-5 lg:flex hidden z-10 relative">
          <div className="w-1/2 transform xl:-translate-y-8 translate-y-2">
            ...
            <p
              className="xl:w-2/3 lg:w-4/5 lg:text-2xl text-xl mt-8 font-medium"
            >
              A good teacher can make or break a class. Here at Polyratings we believe that you should not be rolling
              the dice when you register for a class. This version of Polyratings is a ground up rebuild of the entire website.
          ...
          >
            ...
            <div className="flex mt-2 mb-6">
              ...
              {[...Array(12)].map((_, i) => <img key={i} src={star} alt="star" />)}
            </div>
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-y-14 m-auto mt-20 gap-x-12 xl:gap-x-24 w-[60rem] xl:w-[65rem]">
            {worstTeachers.map((teacher) => <TeacherCard key={teacher.id} teacher={teacher} />)}
          </div>
        </div>
      </div>
    );
