        ...
        <div>
            {
                ...
                <Backdrop>
                    <div className="bg-gray-300 opacity-100 rounded shadow p-5" style={{width:475}}>
                        <EvaluateTeacherForm teacher={teacherData} setTeacher={setTeacherData} closeForm={() => setTeacherEvaluationShownDesktop(false)}/>
                    </div>
                </Backdrop>
            }
            ...
            <div className="container lg:max-w-5xl mx-auto hidden sm:flex justify-between py-2">
                <div>
                    ...
                    <button onClick={() => setTeacherEvaluationShownDesktop(true)} className="bg-cal-poly-green text-white rounded-lg p-2 shadow mt-2">Evaluate Teacher</button>
                </div>
            ...
            <AnimateHeight  duration={500} height={teacherEvaluationShownMobile ? 'auto' : 0}>
                <div className="bg-cal-poly-green text-white p-5">
                    <EvaluateTeacherForm teacher={teacherData} setTeacher={setTeacherData} closeForm={() => setTeacherEvaluationShownMobile(false)}/>
                </div>
            </AnimateHeight>

            {
                Object.entries(teacherData.reviews || {}).map(
                    ([taughtClass, reviews]) => <ClassSection key={taughtClass} reviews={reviews} taughtClass={taughtClass}/>
                )
            }
            <ClassScroll 
...
function ClassSection({reviews, taughtClass}:{reviews:ReviewEntry[], taughtClass:string}) {
    ...
    return (
        <div className="pt-4 relative" id={taughtClass}>
            ...
            <AnimateHeight 
                    duration={500} 
                    height={expanded ? 'auto' : reviews.length > UNEXPANDED_LIMIT ? 25: 0} 
    ...
    return(
        ...
        >
            <div className="hidden lg:flex flex-col w-32 flex-shrink-0 m-auto mr-4 text-center text-sm">
                ...
                <div>{new Date(review.postDate).toLocaleString('en-US', {year: 'numeric', month: 'short'})}</div>
            </div>
