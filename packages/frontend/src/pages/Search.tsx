...
import type { SearchState } from "@/components";
import { ProfessorCard, SearchBar, Filters, PROFESSOR_CARD_HEIGHT_REM } from "@/components";
...
export function Search() {
    ...
    const [searchState, setSearchState] = useLocationState<SearchState>(loadedSearchTerm, "searchState");
    const { data: allProfessors } = trpc.professors.all.useQuery();
    const searchResults = professorSearch(allProfessors ?? [], searchState.type, searchState.searchValue);
    ...
    const rootFontSize = parseFloat(window.getComputedStyle(document.body).getPropertyValue("font-size") || "16");
    ...
    return (
        <div id="main">
            <SearchBar className="mt-7" initialState={searchState} onChange={setSearchState} disableAutoComplete />
