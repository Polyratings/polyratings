import { AddReviewRequest, AddReviewResponse, ProcessingReviewResponse, Teacher } from '@polyratings/shared';
...
export class ReviewService {
  ...
  async uploadReview(addReviewRequest: AddReviewRequest): Promise<Teacher> {
    const addReviewRes = await this.httpService.fetch(`${config.remoteUrl}/professors/${addReviewRequest.professor}/ratings`, {
      method: 'POST',
      headers: {
    ...
    const processingReviewRes = await fetch(`${config.remoteUrl}/ratings/${addReviewResponse.newReviewId}`)
