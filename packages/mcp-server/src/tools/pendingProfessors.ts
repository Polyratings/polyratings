...
export function registerPendingProfessorTools(server: McpServer): void {
    ...
    server.registerTool(
        "polyratings_admin_change_pending_professor_name",
        {
            ...
            inputSchema: {
                pendingProfessorId: pendingProfessorIdField,
                ...nameFields,
            },
            annotations: { readOnlyHint: false, openWorldHint: false },
        },
        adminTool(
            ...
            async ({ pendingProfessorId, firstName, lastName }, token) => {
                await getAdminClient(token).admin.changePendingProfessorName.mutate({
                    professorId: pendingProfessorId,
                    firstName,
                    lastName,
                });
                return {
    ...
    server.registerTool(
        ...
        adminTool(
            ...
            async ({ pendingProfessorId, department }, token) => {
