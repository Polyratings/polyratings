...
For these reasons we decided we needed to associate an identifier with each user of Polyratings that could correlate actions of a period of multiple days.
However, this identifier will ALWAYS be ANONYMOUS, and even we the developers no way to de-anonymize user. We decided to derive this identifier 
from the users IP address but we NEVER store the IP directly anywhere. Instead we use the hash of the IP to associate it with a session id. In addition,
since we are committed to open data we did not want to remove this identifier from the public Github dataset, we needed to make sure that session ids
did not allow for a writing comparison de-anonymization. As such we evaluated two options for implementation:
...
2. Sessions are allocated on a rolling period using KV ttl feature. Effectively any time a user does a write operation to polyratings the time of the session
gets extended by another 7 days. Once the 7 days are up the session id gets deleted. The only disadvantage of this strategy is that a read and write is always
needed since the ttl has to be extended

We decided to go with the second option due to the administrative advantage. To compensate for the performance difference we can decide to not await the write to
the KV. Worst case-scenario the extended TTL is not written to the KV but this is an extremely unlikely outcome since other kv writes will happen in other daos 
after the `getIdentifier` function call.
*/
