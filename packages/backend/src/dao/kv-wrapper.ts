...
export class KvWrapper {
    ...
    async safeGet<T extends ZodTypeAny>(parser: T, key: string): Promise<SafeParseReturnType<T, zodInfer<T>>> {
        const json = await this.kv.get(key, "json");
        return parser.safeParse(json);
    }

    async getOptional<T extends ZodTypeAny>(parser: T, key: string): Promise<zodInfer<T> | undefined> {
        try {
    ...
    async getAll<T extends ZodTypeAny>(parser: T): Promise<zodInfer<T>[]> {
        ...
        const possiblyNullJson = await Promise.all(list.keys.map((key) => this.kv.get(key.name, "json")));
        return possiblyNullJson.filter((exists) => exists).map((json) => parser.parse(json));
    }

    put<T extends ZodTypeAny, U extends zodInfer<T>>(parser: T, key: string, data: U, options?: KVNamespacePutOptions) {
